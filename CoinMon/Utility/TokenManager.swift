class TokenManager {
    ...
    func saveFCMToken(_ token: String) {
        guard let data = token.data(using: .utf8) else { return }
        KeychainHelper.shared.save(data, account: "fcmToken")
    }

    func loadFCMToken() -> String? {
        guard let data = KeychainHelper.shared.load(account: "fcmToken") else { return nil }
        return String(data: data, encoding: .utf8)
    }

    func deleteFCMToken() {
        KeychainHelper.shared.delete(account: "fcmToken")
    }
}
