...
class MarketListAtAlarmCollectionViewCell: UICollectionViewCell {
    ...
    private func updateUI() {
        ...
        alarmCntLabel.textColor = isSelected ? ColorManager.common_100 : ColorManager.gray_60
        self.backgroundColor = isSelected ? ColorManager.gray_22 : ColorManager.gray_99
    }
    
    private func layout(){
        [marketImageView,marketLabel,alarmCntLabel]
            .forEach{
        ...
        alarmCntLabel.snp.makeConstraints { make in
            make.leading.equalTo(marketLabel.snp.trailing).offset(3*ConstantsManager.standardWidth)
            make.trailing.equalToSuperview().offset(-12*ConstantsManager.standardWidth)
            make.centerY.equalToSuperview()
        }
    }
    
    func configure(with market: Market, alarmCount: Int) {
