...
class IndicatorCollectionViewCell: UICollectionViewCell {
    ...
    private func layout(){
        ...
        [alarmSwitch,indicatorTitleLabel,explainButton,premiumLabel,frequencyLabel,separteView,coinImageView,coinTitleLabel,rightButton,coinPriceLabel,alertView,alertTimeLabel,alertPriceLabel,alertTypeLabel]
            .forEach {
    ...
    func configure(with indicatordata: IndicatorCoinData) {
        if LocalizationManager.shared.language == "ko" {
            indicatorTitleLabel.updateAttributedText(indicatordata.indicatorName)
        }
        else {
            indicatorTitleLabel.updateAttributedText(indicatordata.indicatorNameEng)
        }
