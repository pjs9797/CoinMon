...
class UnsavedFavoritesChangeSecondView: UIView {
    let firstLabel: UILabel = {
        let label = UILabel()
        label.font = FontManager.D4_20
        label.textColor = ColorManager.common_0
        label.text = LocalizationManager.shared.localizedString(forKey: "저장하지 않고 이동하시겠어요?")
        return label
    }()
    ...
    let moveButton: UIButton = {
        let button = UIButton()
        button.setTitle(LocalizationManager.shared.localizedString(forKey: "이동하기"), for: .normal)
        button.titleLabel?.font = FontManager.D6_16
    ...
    private func layout() {
        [firstLabel,secondLabel,continueButton,moveButton]
            .forEach{
                addSubview($0)
            }
