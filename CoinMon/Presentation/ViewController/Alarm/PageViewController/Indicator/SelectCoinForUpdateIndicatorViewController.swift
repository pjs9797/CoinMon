...
class SelectCoinForUpdateIndicatorViewController: UIViewController, ReactorKit.View {
    ...
    init(with reactor: SelectCoinForUpdateIndicatorReactor) {
...
extension SelectCoinForUpdateIndicatorViewController {
    func bind(reactor: SelectCoinForUpdateIndicatorReactor) {
        bindAction(reactor: reactor)
        bindState(reactor: reactor)
    }
    
    func bindAction(reactor: SelectCoinForUpdateIndicatorReactor){
        backButton.rx.tap
        ...
        reactor.selectCoinRelay
            .observe(on:MainScheduler.asyncInstance)
            .map{ Reactor.Action.loadUpdateSelectedIndicatorCoin($0) }
            .bind(to: reactor.action)
            .disposed(by: disposeBag)
    }
    
    func bindState(reactor: SelectCoinForUpdateIndicatorReactor){
        ...
        reactor.state.map { $0.checkedCoinList }
            ...
            .bind(to: selectCoinForIndicatorView.selectedCoinForIndicatorCollectionView.rx.items(cellIdentifier: "SelectedCoinForIndicatorCollectionViewCell", cellType: SelectedCoinForIndicatorCollectionViewCell.self)) { index, data, cell in
                ...
                cell.deleteButton.rx.tap
                    .map { Reactor.Action.deleteButtonTapped(indicatorCoinId: String(data.indicatorCoinId)) }
                    .bind(to: reactor.action)
                    .disposed(by: cell.disposeBag)
            }
            .disposed(by: disposeBag)
