struct CoinPriceTranslator {
    static func toPriceListsAtHome(dto: CoinPriceResponseDTO) -> [CoinPrice] {
        return dto.data.info.compactMap { CoinPriceInfoTranslator.toPriceListAtHome(dto: $0) }
    }
    
    static func toPremiumLists(dto: CoinPriceResponseDTO) -> [OneCoinPrice] {
        return dto.data.info.compactMap { CoinPriceInfoTranslator.toPremiumList(dto: $0) }
    }
    
    static func toPriceListsAtAlarm(dto: CoinPriceResponseDTO) -> [OneCoinPrice] {
        return dto.data.info.map { CoinPriceInfoTranslator.toPriceListAtAlarm(dto: $0) }
    }
    
    static func toOnePriceListsAtAlarm(dto: CoinPriceResponseDTO) -> OneCoinPrice {
        return dto.data.info.first.map { CoinPriceInfoTranslator.toPriceListAtAlarm(dto: $0) } ?? OneCoinPrice(coinTitle: "AAA", price: "12345")
    }
    
    static func toFeeList(dto: CoinPriceResponseDTO) -> [CoinFee] {
        return dto.data.info.compactMap { CoinPriceInfoTranslator.toFeeList(dto: $0) }
    }
}

struct CoinPriceInfoTranslator {
